...
load_azd_environment() {
    ...
    if is_azd_available; then
        ...
        export RESOURCE_GROUP=$(get_env_value "AZURE_RESOURCE_GROUP" "$RESOURCE_GROUP")
        export CONTAINER_APP_NAME=$(get_env_value "AZURE_CONTAINER_APP_NAME" "$CONTAINER_APP_NAME")
        export CONTAINER_APP_URI=$(get_env_value "AZURE_CONTAINER_APP_URI" "$CONTAINER_APP_URI")
        export REDIS_HOST=$(get_env_value "AZURE_REDIS_HOST" "$REDIS_HOST")
        export LOCATION=$(get_env_value "AZURE_LOCATION" "$LOCATION")
