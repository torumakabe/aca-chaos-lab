...
class BaselineUser(HttpUser):
            ...
            if response.status_code != 200:
                ...
                try:
                    ...
                    if data.get("load", {}).get("active"):
                        logger.warning(
                            "Load simulation is active during baseline test!"
                        )
                    if data.get("hang", {}).get("active"):
                        logger.warning(
                            "Hang simulation is active during baseline test!"
                        )
                except Exception as e:
    ...
    def on_stop(self):
        ...
        logger.info(
            f"Baseline user session ended - Requests: {self.request_count}, Errors: {self.error_count} ({error_rate:.2f}%)"
        )
