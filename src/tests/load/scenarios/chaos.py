...
class ChaosInjector:
    ...
    def __init__(self, host: str):
        self.host = host
        self.active_chaos = {"load": False, "network": False, "deployment": False}
    ...
    def inject_network_chaos(self):
        ...
        self.injection_history.append(
            {"type": "network", "timestamp": time.time(), "success": True}
        )
            ...
            if response.status_code != 200:
                ...
                try:
                    ...
                    if data.get("redis_data") == "Redis unavailable":
                        if (
                            chaos_injector
                            and "network" in chaos_injector.get_active_chaos()
                        ):
            ...
            if response.status_code == 200:
                try:
                    ...
                    if data["status"] == "unhealthy":
                        active_chaos = (
                            chaos_injector.get_active_chaos() if chaos_injector else []
                        )
        ...
        with self.client.get("/chaos/status", name="Chaos Status") as response:
            if response.status_code == 200:
                try:
                    ...
                    if chaos_injector:
                        chaos_injector.active_chaos["load"] = data.get("load", {}).get(
                            "active", False
                        )
                except:
                    pass
...
def on_test_start(environment: Environment, **kwargs):
    ...
    chaos_injector = ChaosInjector(host)
...
def on_locust_init(environment: Environment, **kwargs):
    ...
    if not isinstance(environment.runner, type(None)):
        ...
        def schedule_checks():
            for _ in range(200):
                check_and_inject_chaos()
                time.sleep(5)

        environment.runner.greenlet.spawn(schedule_checks)
    ...
    if chaos_injector:
        ...
        for injection in chaos_injector.injection_history:
            timestamp = time.strftime(
                "%H:%M:%S", time.localtime(injection["timestamp"])
            )
            print(
                f"  - {timestamp}: {injection['type']} "
