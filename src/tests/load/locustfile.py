...
class ChaosLabUser(HttpUser):
    ...
    def check_health(self):
        ...
        with self.client.get("/health", catch_response=True) as response:
            if response.status_code == 200:
                ...
                response.failure(
                    "Health check failed with status 0 - Connection error or timeout"
                )
            else:
                response.failure(
    ...
    def main_endpoint(self):
        ...
        with self.client.get("/", catch_response=True) as response:
            if response.status_code == 200:
                ...
                response.failure(
                    "Main endpoint failed with status 0 - Connection error or timeout"
                )
            else:
                response.failure(
                    f"Main endpoint failed with status {response.status_code}"
                )
    ...
    def check_chaos_status(self):
        ...
        with self.client.get("/chaos/status", catch_response=True) as response:
            if response.status_code == 200:
                data = response.json()
                self.chaos_active = data.get("load", {}).get(
                    "active", False
                ) or data.get("hang", {}).get("active", False)
                response.success()
            elif response.status_code == 0:
                response.failure(
                    "Chaos status check failed with status 0 - Connection error or timeout"
                )
            else:
                response.failure(
                    f"Chaos status check failed with status {response.status_code}"
                )
...
def on_test_start(environment: Environment, **kwargs):
    ...
    if environment.parsed_options:
        ...
        print("No parsed options available")
