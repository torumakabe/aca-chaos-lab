...
async def load_generator(level: str, duration: int):
    ...
    try:
        ...
        await asyncio.gather(
            generate_cpu_load(level, duration),
            generate_memory_load(level, duration)
        )
...
async def start_load(request: LoadRequest):
    ...
    if request.level not in ["low", "medium", "high"]:
        raise HTTPException(status_code=400, detail="Invalid load level. Must be 'low', 'medium', or 'high'")
    ...
    logger.info(f"Load simulation started: level={request.level}, duration={request.duration_seconds}s")
...
async def hang(request: HangRequest):
    ...
    if request.duration_seconds > 0:
        chaos_state.hang_end_time = datetime.now(UTC) + timedelta(seconds=request.duration_seconds)
    else:
...
async def get_status():
    ...
    return ChaosStatusResponse(
        ...
        hang={
            "active": chaos_state.hang_active,
            "remaining_seconds": hang_remaining
        }
    )