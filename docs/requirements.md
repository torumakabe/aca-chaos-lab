...
- 専用のハングアップAPIエンドポイントを提供
- APIが呼び出されたとき、レスポンスを返さずに処理を無限に待機
- ヘルスチェックエンドポイントへの影響（オプション）
- 設定可能なハングアップ時間（永続的または時限的）
...
- 障害注入方法（NSGルール操作、負荷制御API、ハングアップAPI、リビジョン更新）
